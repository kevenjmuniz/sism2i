...
@app.post("/api/clients/{client_id}/hide-supplier")
def hide_supplier(client_id: int):
    try:
        init_db()
        with get_connection() as conn:
            conn.execute("UPDATE clientes SET is_fornecedor = ? WHERE id = ?", (True, client_id))
        supabase_store.invalidate_snapshot_cache()
        return {"hidden": True, "client_id": client_id}
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Não foi possível ocultar o fornecedor: {exc}") from exc
